...
def train_set():
    ...
    noise_level = 0.25
    with open("/vol/research/mightywings/t2a/niv/train75_set.csv", 'w', newline='') as outFile:
        header = ['NAME', 'ROW', 'COL', 'RAD']
        write(outFile, header)
        for i in tqdm(range(number_of_images)):
            img, params = generate_examples(noise_level=noise_level)
            np.save("/vol/research/mightywings/t2a/niv/datasets/train75/" + str(i) + ".npy", img)
            write(outFile, ["/vol/research/mightywings/t2a/niv/datasets/train75/" + str(i) + ".npy", params.row, params.col, params.radius])
